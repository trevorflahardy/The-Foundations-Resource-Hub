...
No one gets it right on the first try. You should expect to encounter failures, but it is important that you learn from them. This section assumes you've already prepared your model as described in :ref:`preparing_to_print`.

If something is not working, do not just try again. Instead, take a step back and analyze the problem. Start by checking for these common issues, then adjust your design or slicing settings accordingly.
...
**Symptom:**
Corners lifting off the build plate, or sometimes the entire print lifting from the build plate completely.

**Possible causes:**
Poor first-layer adhesion; build plate temperature too low.

**What to try:** (in order)
  #. Add a one-layer brim in slicer settings to expand contact area.
  #. Increase build plate temperature by 5 °C in your slicer settings.
...
.. figure:: ../images/3d_printing/warping_problem.jpg
   :alt: 3D print showing warping with corners lifting off the build plate
   :figwidth: 50%

   Warping where corners have lifted from the build plate. [#simplify3d]_

Layer Shifts
^^^^^^^^^^^^

**Symptom:**
Layers misaligned horizontally during printing.
...
#. **Adjust**
   Tweak your CAD model or slicer profile slightly: wall thickness, bridging angles, infill percentage, etc.
#. **Repeat**
...
Once you are comfortable with troubleshooting, explore turning "working" designs into "optimized" designs:
